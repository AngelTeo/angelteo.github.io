// LoveGo v42 · Student Override management
// Base rule stays CLASS assignment. Overrides are scoped to one department/class reporting context.
(()=>{
  const wait=()=>{
  ...
  function install(){
    ...
    function currentRows(classId,studentId){
      const cyc=CLOUD.cycleMap[S.ui.cycle];
      return (S.cloudAssignments||[]).filter(a=>a.class_id===classId&&a.student_id===studentId&&(!cyc||a.cycle_id===cyc.id)&&!a.archived_at&&['active','pending','completed'].includes(a.status)).sort((a,b)=>a.slot_no-b.slot_no);
    }
    ...
    window.openStudentOverrides=async function(classId){
      ...
      document.getElementById('overrideTitle').textContent=`个别学生调整 · ${cl?.department||''} · ${cl?.display_name||cl?.class_name||''}`;
      ...
      try{
        ...
        body.innerHTML=`<div class="alert"><b>Reporting Context</b> · ${cl?.department||''} / ${cl?.display_name||cl?.class_name||''}<br>这里的调整不会影响学生在其他部门的报告。</div><div class="card"><div class="title">选择学生</div><select id="overrideStudent" style="width:100%"><option value="">— 选择需要例外调整的学生 —</option>${students.map(s=>`<option value="${s.id}">${s.name_en||s.name_cn||s.id}${s.name_cn&&s.name_en?' · '+s.name_cn:''}${ovByStudent[s.id]?.length?' · 已有例外':''}</option>`).join('')}</select></div><div id="overrideStudentDetail"></div>`;
    ...
    window.applyStudentOverride=async function(classId,studentId,slot){
      ...
      const reason=prompt('请简短记录为什么这名学生在这个班 / 部门需要由不同老师负责：')||'';
      if(!reason.trim()){toast('Student Override 必须记录原因');return;}
      try{
        const q=await sb.rpc('lovego_set_student_override',{p_cycle_id:cyc.id,p_class_id:classId,p_student_id:studentId,p_slot_no:slot,p_new_teacher_id:sel.value,p_reason:reason.trim()});
    ...
    window.revertStudentOverride=async function(classId,studentId,slot){
      const cyc=CLOUD.cycleMap[S.ui.cycle];if(!cyc)return;
      const reason=prompt('请记录恢复这个班级默认 Assignment 的原因：')||'';
      if(!reason.trim()){toast('恢复也必须留下原因');return;}
      try{
        const q=await sb.rpc('lovego_revert_student_override',{p_cycle_id:cyc.id,p_class_id:classId,p_student_id:studentId,p_slot_no:slot,p_reason:reason.trim()});
        if(q.error)throw q.error;
